...
def is_approver(username: str):
    ...
    if "approvers" not in out:
        raise Exception('OWNERS file did not have the "approvers" key and it is required')

    approvers = out.get('approvers')
    if type(approvers) is not list:
...
def main(): 
    if len(sys.argv) != 2:
        print('[Error] This script accepts only a single string as an argument, representing the user to check.')
        return 10
